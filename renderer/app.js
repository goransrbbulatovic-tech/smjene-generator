...
(() => {
  ...
  function loadFromData(data) {
    ...
    setTimeout(() => {
      ...
      _exportSelectedPersons = new Set(people());
  ...
  function goToPage(id) {
    ...
    if (id==='export') { renderExportPersonList(); renderExportPersonChips(); }
  }
  ...
  function processBase64(b64, fileName, filePath) {
    try {
      ...
      updateAutoImportBar();
      renderExportPersonList();
      _exportSelectedPersons = new Set(p.people || []);
      renderExportPersonChips();
  ...
  function copyWhatsApp() {
    const selectedPersons = [..._exportSelectedPersons];
  ...
  function setupSettingsPage() {
    ...
    if (n24El) n24El.onchange = () => {
      ...
      persistAll();
      showToast(n24El.checked ? '🔔 Notifikacije 24h uključene' : '🔕 Notifikacije 24h isključene');
    };
    if (n1El) n1El.onchange = () => {
      state.settings.notif1 = n1El.checked;
      persistAll();
      showToast(n1El.checked ? '🔔 Notifikacije 1h uključene' : '🔕 Notifikacije 1h isključene');
    };
    // Sound hover toggle
    const soundHoverEl = document.getElementById('sound-hover');
    if (soundHoverEl) {
      soundHoverEl.checked = state.settings.soundHover !== false;
      soundHoverEl.onchange = () => {
        state.settings.soundHover = soundHoverEl.checked;
        persistAll();
        showToast(soundHoverEl.checked ? '🔊 Zvuk pri hoveru uključen' : '🔇 Zvuk pri hoveru isključen');
      };
    }
  ...
  let _exportSelectedPersons = new Set();
  ...
  function renderExportPersonChips() {
    ...
    // Init selection to all persons if empty
    if (_exportSelectedPersons.size === 0) ppl.forEach(n => _exportSelectedPersons.add(n));

    chips.innerHTML = ppl.map(name => {
      const pc  = personColor(name);
      const sel = _exportSelectedPersons.has(name);
      const ini = name.split(/\s+/).map(w=>w[0]).join('').slice(0,2);
    ...
    chips.querySelectorAll('.eps-chip').forEach(chip => {
      chip.onclick = () => {
        const name = chip.dataset.person;
        const pc   = personColor(name);
        if (_exportSelectedPersons.has(name)) {
          _exportSelectedPersons.delete(name);
          chip.classList.remove('selected');
          chip.style.borderColor = ''; chip.style.color = '';
        } else {
          _exportSelectedPersons.add(name);
          chip.classList.add('selected');
  ...
  function setupExportPersonButtons() {
    ...
    if (selAll) selAll.onclick = () => {
      people().forEach(n => _exportSelectedPersons.add(n));
      renderExportPersonChips();
    };
    if (selNone) selNone.onclick = () => {
      _exportSelectedPersons.clear();
      renderExportPersonChips();
    };
  }

  function setupExportPage() {
    setupExportPersonButtons();
    document.getElementById('btn-export-excel').onclick = async () => {
      const selectedPersons = [..._exportSelectedPersons];
      if (!selectedPersons.length) { alert('Odaberi bar jednu osobu!'); return; }
      const opts = {
        allPersons: selectedPersons.length > 1,
        stats: document.getElementById('exp-stats')?.checked ?? true,
      };
      const b64  = Exporter.exportExcel(shifts(), selPerson()||selectedPersons[0], opts, selectedPersons);
      const name = selectedPersons.length === 1
        ? `smjene_${selectedPersons[0].replace(/\s+/g,'_')}.xlsx`
        : `smjene_sve_osobe_${new Date().toISOString().slice(0,10)}.xlsx`;
    ...
    document.getElementById('btn-export-pdf').onclick = async () => {
      ...
      const avatar = getAvatar(selPerson()||selectedPersons[0]);
      const html   = Exporter.buildPrintHTML(
        shifts(), selPerson()||selectedPersons[0],
