...
(() => {
  ...
  function setupExportPage() {
    try { setupExportPersonButtons(); } catch(e) { console.error('setupExportPersonButtons:', e); }

    const excelBtn = document.getElementById('btn-export-excel');
    const pdfBtn   = document.getElementById('btn-export-pdf');
    const waBtn    = document.getElementById('btn-copy-whatsapp');

    if (excelBtn) excelBtn.onclick = async () => {
      try {
        if (!state.exportSelPersons.length) state.exportSelPersons = people().slice();
        const selectedPersons = [...state.exportSelPersons];
        if (!selectedPersons.length) { showToast('⚠️ Nema osoba — uvezi fajl!'); return; }
        const statsChecked = document.getElementById('exp-stats')?.checked ?? true;
        const opts = { allPersons: selectedPersons.length > 1, stats: statsChecked };
        const allShifts = shifts();
        const b64  = Exporter.exportExcel(allShifts, selectedPersons[0], opts, selectedPersons);
      ...
      } catch(err) { console.error('Excel export error:', err); showToast('❌ Greška: ' + err.message); }
    };
    ...
    if (pdfBtn) pdfBtn.onclick = async () => {
      try {
        ...
        showToast('⏳ Generisanje PDF-a...');
        if (window.api) {
          document.getElementById('print-content').innerHTML = html;
          const ok = await window.api.exportPdf({ defaultName: name });
          if (ok) showToast('✅ PDF sačuvan!');
        } else {
          const w = window.open(''); w.document.write(html); w.document.close();
          setTimeout(() => w.print(), 300);
        }
      } catch(err) { console.error('PDF export error:', err); showToast('❌ Greška: ' + err.message); }
    };

    if (waBtn) waBtn.onclick = copyWhatsApp;
  }
