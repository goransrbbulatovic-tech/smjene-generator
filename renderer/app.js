...
(() => {
  ...
  function goToPage(id) {
    ...
    if (id==='export') {
      // Sync selection with current people
      // Remove stale persons, but only auto-select-all if never visited before
      const validSel = state.exportSelPersons.filter(n => people().includes(n));
      const ppl = people();
      // If nothing selected AND there are people available, default to all selected
      state.exportSelPersons = validSel.length > 0 ? validSel : ppl.slice();
      renderExportPersonList();
      setTimeout(renderExportPersonChips, 30);
    }
  }
  ...
  window.exportSelAll = function() {
    state.exportSelPersons = people().slice();
    renderExportPersonChips();
    showToast('✓ Sve osobe odabrane');
  };

  window.exportSelNone = function() {
    // Mark as explicitly cleared — use sentinel so render doesn't auto-fill
    state.exportSelPersons = [];
    state._exportSelCleared = true;
    renderExportPersonChips();
    showToast('✗ Selekcija poništena');
  };

  window.doExportExcel = async function() {
    try {
      const allPeople = people();
      if (!allPeople.length) { showToast('⚠️ Nema osoba — uvezi fajl!'); return; }
      // Export currently selected person, or all people
      const person = selPerson();
      const selectedPersons = person ? [person] : allPeople;
      ...
      const name = person
        ? `smjene_${person.replace(/\s+/g,'_')}.xlsx`
  ...
  window.doExportPdf = async function() {
    try {
      ...
      const name = person
        ? `smjene_${person.replace(/\s+/g,'_')}.pdf`
  ...
  window.renderExportPersonChips = renderExportPersonChips;
  function renderExportPersonChips() {
    ...
    // Remove stale persons; respect user's explicit empty selection
    state.exportSelPersons = state.exportSelPersons.filter(n => ppl.includes(n));
    // Only auto-select-all if user hasn't explicitly cleared selection
    if (!state._exportSelCleared && state.exportSelPersons.length === 0) {
      state.exportSelPersons = ppl.slice();
    }
